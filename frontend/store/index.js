...
const actions = {
  [types.REGISTER]: ({ commit }, payload) => {
    axios.post(USERS_URL, payload).then((r) => {
      //console.log(r);
    });
  },
  [types.UPDATE_TOKEN]: ({ commit }, payload) => {
    return new Promise((resolve) => {
      axios
        .post(OBTAIN_URL, payload.credentials)
        .then((r) => {
          console.log(r);
          if (r.data.access) {
            const mutationPayload = {
              ...
              user: JSON.parse(atob(r.data.access.split(".")[1])),
            };
  ...
  [types.INSPECT_TOKEN]: ({ dispatch, state }) => {
    return new Promise((resolve, reject) => {
      ...
      if ((exp - now < 1800 && now - orig_iat < 628200) || true) {
        dispatch(types.REFRESH_TOKEN).then((token) => resolve(token.access));
      } else {
  ...
  [types.UPDATE_CHILD]: ({ commit }, payload) => {
    axios
      .patch(CHILDREN_URL + payload.id + "/", payload)
      .then((r) => {
        if (r.data) {
  ...
  [types.GET_HOLIDAYS_SECTION_CAPACITY]: ({ commit }, payload) => {
    ...
    axios
      .get(`${HOLIDAYS_URL}${payload.holiday_id}/get_capacity`)
      .then((r) => {
        if (r.data) {
  ...
  [types.CREATE_REGISTRATION]: ({ commit, dispatch }, payload) => {
    axios
      .post(REGISTRATIONS_URL, payload)
      .then((r) => {
        if (r.data) {
  ...
  [types.VERIFY_FORGOT_PASSWORD_TOKEN]: ({}, payload) => {
    return new Promise((resolve) => {
      axios
        .post(FORGET_PASSWORD_URL + "validate_token/", {
          token,
        })
        .then((r) => {
  ...
  [types.RESET_PASSWORD]: ({}, payload) => {
    return new Promise((resolve) => {
      axios
        .post(FORGET_PASSWORD_URL + "confirm/", {
          token: payload.token,
    ...
    return new promise((resolve) => {
      axios
        .get(
          HOLIDAYS_URL +
            payload.holiday_id +
            "/get_section_for_child?child_id=" +
            payload.child_id
        )
        .then((r) => {
